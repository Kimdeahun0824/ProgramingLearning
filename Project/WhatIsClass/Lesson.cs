...
namespace WhatIsClass
{
    internal class Lesson
    {
        ...
        {
            /**
             * C#의 모든 코드에 반디스 들어가는 클래스(Class) 아라보자.
             * 
             * 클래스 소개하기
             ...
             *      - 무언가 내용
             * }
             * 같은 코드 블록을 사용하여 정의할 수 있다. 클래스의 구성 요소는 많지만, 그 중 속성과 메서드를 가장 많이 사용한다.
             * 속성은 데이터를 다루고, 메서드는 로직을 다룬다. 
             * - 클래스
             *  - 속성 : 데이터
             *  - 메서드 : 로직
             * 
             * 클래스는 그 의미에 따라, 이미 닷넷 프레임워크에서 만들어 놓은 내장 형식(bulit-in type)과 사용자가 직접
             * 클래스 구조를 만드는 사용자 정의 형식(User Defined Type)으로 구분할 수 있다. 예를 들어
             * Console, String, Math, 등 클래스는 내장 형식이다. 그리고 Class 키워드로 Product, Note, User, Group 처럼
             * 새로운 형식(기존에 제공되지 않는)을 정의할 수 있는데, 이를 사용자 데이터 형식 이라고 한다.
             * 
             * 클래스 만들기
             * 클래스를 정의하면 다음과 같다.
             ...
             *  개체를 생성하는 틀(템플릿)이며, 더 간단히 말하자면 "무엇이간를 만들어 내는 설계도" 이다.
             *  
             *  클래스 선언하기
            ...
            /**
             * 클래스를 여러 개 사용할 떄는 public 키워드를 써야 한다. public 키워드가 붙은 클래스는
             ...
             * 언제든지 접근 가능하다. 하지만 학습 단계에서는 클래스에 public만 사용해도 괜찮다. 그리고 class 키워드
             * 다음에 클래스 이름이 오는데, 클래스 이름은 대문자로 시작하는 명사를 사용한다.
            ...
            //int[] lottoNum = lottoNumGenerator.GetLottoNum();
            //int bonusNum = lottoNum[lottoNum.Length - 1];
            ////Array.Sort(lottoNum);
            //for (int i = 0; i < 6; i++)
            //{
            //    Console.WriteLine(lottoNum[i].ToString());
            //}
            //Console.WriteLine(bonusNum.ToString());
        }
